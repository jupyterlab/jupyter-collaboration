...
def install_dev() -> None:
    install_build_deps = "python -m pip install jupyterlab>=4.4.0,<5 hatchling>=1.4.0 hatch-jupyter-builder>=0.5 editables"
    ...
    for py_package in python_packages:
        ...
        execute(f"pip install --no-build-isolation -e {python_package_prefix}/{py_package}[test]")
